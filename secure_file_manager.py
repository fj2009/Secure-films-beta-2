...
def print_banner() -> None:
    # El emoji se muestra aparte del cálculo de ancho: su anchura en pantalla
    # varía según la terminal/fuente y rompería la alineación del marco.
    ...
    empty_line = f"║{' ' * width}║"
    title_line = f"║  {('🔐 ' + title).center(width - 4)}  ║" if _supports_color() else f"║  {title.center(width - 4)}  ║"
    subtitle_line = f"║  {subtitle.center(width - 4)}  ║"

    print(_stylize(border_top, TextStyle.CYAN))
    print(_stylize(empty_line, TextStyle.CYAN))
    print(_stylize(title_line, TextStyle.CYAN + TextStyle.BOLD))
    print(_stylize(subtitle_line, TextStyle.CYAN))
    print(_stylize(empty_line, TextStyle.CYAN))
    print(_stylize(border_bottom, TextStyle.CYAN))
    print(_stylize(
        "Gestiona usuarios, archivos cifrados y permisos de acceso de forma segura.",
        TextStyle.GREEN,
    ))
    print(_stylize("Selecciona una opción para comenzar.\n", TextStyle.DIM))
...
def _pbkdf2_hash(password: str, salt: bytes) -> str:
    digest = hashlib.pbkdf2_hmac("sha256", password.encode("utf-8"), salt, 200_000, dklen=32)
...
class SecureAccessManager:
    ...
    def _load(self) -> None:
        ...
        if self.master_key_file.exists():
            self.master_key = self.master_key_file.read_bytes()
    ...
    def _save(self) -> None:
        ...
        if self.master_key is not None:
            self.master_key_file.write_bytes(self.master_key)
    ...
    def decrypt_file(self, file_name: str, username: str, password: str, output_path: Optional[Path] = None) -> Path:
        ...
        if output_path is None:
            output_path = Path.cwd() / record.original_name
...
def build_parser() -> argparse.ArgumentParser:
    ...
    encrypt_parser.add_argument("--remove-original", action="store_true", help="Eliminar el archivo original después de cifrar")
    encrypt_parser.add_argument("--yes", action="store_true", help="No pedir confirmación al eliminar el original")

    decrypt_parser = subparsers.add_parser("decrypt", help="Descifrar un archivo para un usuario autorizado")
    decrypt_parser.add_argument("--file", required=True, help="Nombre original, ID o nombre almacenado del archivo seguro")
    decrypt_parser.add_argument("--username", required=True, help="Usuario que solicita el acceso")
    decrypt_parser.add_argument("--output", help="Directorio o ruta de salida para el archivo descifrado")
    decrypt_parser.add_argument("--yes", action="store_true", help="Sobrescribir el archivo de salida sin preguntar")
...
def prompt_value(prompt_text: str) -> str:
    while True:
        try:
            value = input(_stylize(f"{prompt_text}: ", TextStyle.YELLOW)).strip()
        except (EOFError, KeyboardInterrupt):
            print()
...
def print_menu() -> None:
    print(_stylize("\nMenú principal", TextStyle.BOLD + TextStyle.CYAN))
    options = [
    ...
    for number, description in options:
        print(_stylize(f"  {number}. {description}", TextStyle.CYAN))
...
def run_interactive_menu(manager: SecureAccessManager) -> int:
    while True:
        ...
        if choice == "1":
            ...
            admin = prompt_value("Usuario administrador")
            admin_password = prompt_password(f"Contraseña del administrador '{admin}': ")
            file_path = prompt_value("Ruta del archivo a cifrar")
            ...
            manager.encrypt_file(admin, admin_password, Path(file_path), allowed_users)
        elif choice == "6":
            username = prompt_value("Usuario")
...
def main() -> int:
    ...
    try:
        if args.command == "init":
            ...
            if args.remove_original and not args.yes:
            ...
            manager.encrypt_file(args.admin, admin_password, Path(args.file), allowed, remove_original=args.remove_original)

        elif args.command == "decrypt":
            user_password = prompt_password(f"Contraseña del usuario '{args.username}': ")
            output = Path(args.output) if args.output else None
            candidate = output if output and not (output.exists() and output.is_dir()) else None
            if candidate and candidate.exists() and not args.yes:
                if not confirm_action(f"El archivo '{candidate}' ya existe. ¿Sobrescribirlo?"):
                    info("Operación cancelada.")
                    return 0
            manager.decrypt_file(args.file, args.username, user_password, output_path=output)
